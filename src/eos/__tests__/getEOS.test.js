...
test('wrong intitialization', () => {
  ...
  expect(() => {
    return getProperties(methane, 298, { pressure: null, volume: null });
  }).toThrow('You need to specify pressure or volume!');

  expect(() => {
    return getProperties(methane, 298, { pressure: 1, volume: 1 });
  }).toThrow('You need to specify pressure OR volume, not both!');
});

test('methane', () => {
  const methane = MolecularFluid.fromName('methane');
  let res = getProperties(methane, 290, { pressure: 100 });
...
test('methane close to saturation', () => {
  const methane = MolecularFluid.fromName('methane');
  let res = getProperties(methane, 92.15, { pressure: 2.20178 });
...
test('hexane', () => {
  ...
  let res = getProperties(hexane, 322.29, { pressure: 0.5 });
  expect(res.zList).toHaveLength(2);
